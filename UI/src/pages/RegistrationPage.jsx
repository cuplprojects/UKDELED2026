...
export default function RegistrationPage() {
      ...
      if (token) {
        try {
          ...
          if (res.data && res.data.length > 0) {
            ...
            // If user completed step 4, they're locked - form is now locked after payment
            if (lastStep >= 4) {
              setIsLocked(true);
            }
        ...
        try {
          ...
          if (res.data && res.data.success && res.data.data) {
            ...
            if (d.personalDetailId) {
              ...
              setFormData((prev) => {
                const stateVal = d.state || prev.state || "Uttarakhand";
            ...
            // If CompletedStep from API is 4 or higher, lock editing (form is locked after payment)
  ...
  const handleInputChange = (e) => {
    ...
    } else if (e.target.name === "deled1UdiseCode" || e.target.name === "deled2UdiseCode") {
      let val = value.replace(/\D/g, "");
      if (val.length > 0 && !val.startsWith("05")) {
        if (val.startsWith("0") || val.startsWith("5")) {
    ...
    setFormData((prev) => {
      ...
      if (e.target.name === "subCategory" && value !== "EX-SERVICEMAN (Self)") {
        updated.retirementDate = "";
      }
      if (
        e.target.name === "firstLanguage" &&
        updated.secondLanguage === value
      ) {
        updated.secondLanguage = "Select";
      }
      if (e.target.name === "deled1TrainingQualification") {
        const qual = value;
        const code = updated.eligibilityCodeDELED1;
        if (qual === "In Service Teachers") {
          updated.deled1TrainingStatus = "PASSED";
        }
        if (qual !== "In Service Teachers") {
          updated.deled1InServiceTraining = "Select";
          updated.deled1InServiceTrainingOthers = "";
        }
        if (qual === "In Service Teachers" && code !== "7") {
          updated.eligibilityCodeDELED1 = "Select";
        } else if (qual === "D.El.Ed" && !["1", "2", "5", "6"].includes(code)) {
          updated.eligibilityCodeDELED1 = "Select";
        } else if (qual === "B.El.Ed" && code !== "3") {
          updated.eligibilityCodeDELED1 = "Select";
        } else if (qual === "D.Ed. (Sp.Edu.)" && code !== "4") {
          updated.eligibilityCodeDELED1 = "Select";
        } else if (qual === "B.T.C." && !["1", "2", "5"].includes(code)) {
          updated.eligibilityCodeDELED1 = "Select";
        }
      }
      if (e.target.name === "deled1InServiceTraining" && value !== "Others") {
        updated.deled1InServiceTrainingOthers = "";
      }
      if (e.target.name === "deled2TrainingQualification") {
        const qual = value;
        const code = updated.eligibilityCodeDELED2;
        if (qual === "In Service Teachers") {
          updated.deled2TrainingStatus = "PASSED";
        }
        if (qual !== "In Service Teachers") {
          updated.deled2InServiceTraining = "Select";
          updated.deled2InServiceTrainingOthers = "";
        }
        if (qual === "In Service Teachers" && code !== "8") {
          updated.eligibilityCodeDELED2 = "Select";
        } else if ((qual === "B.T.C." || qual === "D.El.Ed") && code !== "1") {
          updated.eligibilityCodeDELED2 = "Select";
        } else if (qual === "B.Ed." && !["2", "3", "7"].includes(code)) {
          updated.eligibilityCodeDELED2 = "Select";
        } else if (
          (qual === "L.T." || qual === "Shiksha Shastri") &&
          !["2", "3"].includes(code)
        ) {
          updated.eligibilityCodeDELED2 = "Select";
        } else if (qual === "B.El.Ed" && code !== "4") {
          updated.eligibilityCodeDELED2 = "Select";
        } else if (
          (qual === "B.A. Edu" || qual === "B.Sc. Edu.") &&
          code !== "5"
        ) {
          updated.eligibilityCodeDELED2 = "Select";
        } else if (qual === "B.Ed. (Sp.Edu.)" && code !== "6") {
          updated.eligibilityCodeDELED2 = "Select";
        }
      }
      if (e.target.name === "deled2InServiceTraining" && value !== "Others") {
        updated.deled2InServiceTrainingOthers = "";
      }
      if (e.target.name === "eligibilityCodeDELED1") {
        if (value === "7" && !updated.deled1UdiseCode) {
          updated.deled1UdiseCode = "05";
        } else if (value !== "7") {
          updated.deled1UdiseCode = "";
          updated.deled1SchoolType = "Select";
        }
      }
      if (e.target.name === "eligibilityCodeDELED2") {
        if (value === "8" && !updated.deled2UdiseCode) {
          updated.deled2UdiseCode = "05";
        } else if (value !== "8") {
          updated.deled2UdiseCode = "";
          updated.deled2SchoolType = "Select";
        }
      }
      ...
      if (e.target.name === "applyFor") {
        const typeId = getExamTypeId(value, updated.category, updated.phyHandicapped);
        const isDeled1Req = [1, 4, 3, 6].includes(typeId);
        const isDeled2Req = [2, 5, 3, 6].includes(typeId);
        
        if (!isDeled1Req) {
          updated.deled1TrainingQualification = "Select";
          updated.deled1TrainingStatus = "Select";
          updated.deled1TrainingYear = "Select";
          updated.eligibilityCodeDELED1 = "Select";
          updated.deled1InServiceTraining = "Select";
          updated.deled1InServiceTrainingOthers = "";
          updated.deled1UdiseCode = "";
        ...
        if (!isDeled2Req) {
          updated.deled2TrainingQualification = "Select";
          updated.deled2TrainingStatus = "Select";
          updated.deled2TrainingYear = "Select";
          updated.eligibilityCodeDELED2 = "Select";
          updated.deled2InServiceTraining = "Select";
          updated.deled2InServiceTrainingOthers = "";
          updated.deled2UdiseCode = "";
          updated.deled2SchoolType = "Select";
          updated.subjectCode = "Select";
        }
      }
    ...
    if (step === 3) {
      if (!formData.agreedTerms) {
        notification.warning({
          ...
          description:
            "Please agree to the Terms & Conditions (घोषणा) to proceed.",
        });
        return;
      }
        ...
        <div className="w-full max-w-[1550px] grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
          ...
          <div className="md:col-span-9 lg:col-span-9 flex flex-col border border-gray-300 rounded-xs overflow-hidden bg-white shadow-xs">
            <div className="bg-[#0872a4] text-white px-4 py-2.5 flex items-center gap-2.5">
              ...
              <h1 className="text-base md:text-lg font-bold tracking-wide">New Application</h1>
            </div>
            ...
            <div className="bg-white p-4 sm:p-6 md:p-8 overflow-x-hidden">
              ...
              {step === 4 && (
                <PaymentStep
                  formData={formData}
                  onBackToPreview={handlePrevious}
                  examTypes={examTypes}
                  onProceedPayment={() => { }}
                  isLocked={isLocked}
                />
